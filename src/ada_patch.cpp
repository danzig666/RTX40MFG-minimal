...
const TemporalProviderProfile* ProfileForVersion(
    provider_policy::VersionTriplet version) noexcept
{
...
bool ObserveAdapterLuid(const LUID& luid, const wchar_t* api) noexcept
{
    ...
    const uint64_t packedLuid = PackLuid(luid);
    {
        ...
        if (gProvider && gPublishedAdapterLuid != packedLuid)
        {
            ...
            Log(L"Ada temporal patch requires restart after an adapter change");
            return false;
        }
    ...
    Log(L"%s adapter verification: luid=0x%016llX "
...
bool PatchProvider(HMODULE module, const wchar_t* suppliedPath) noexcept
{
    ...
    provider_policy::VersionTriplet providerVersion{};
    if (!provider_policy::ReadProviderVersion(path, providerVersion))
        return fail(Failure::eProviderVersion);
    ...
    if (!published.success)
    {
        ...
        Log(L"Ada temporal patch publication failed: path=%s", path);
        return false;
    }
    ...
    Log(L"Ada temporal patch published at provider RVA 0x%zX: %s",
